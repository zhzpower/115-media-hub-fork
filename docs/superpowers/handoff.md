...
- 2026-05-21 21:26 CST | main | 优化频道排序拖拽交互：排序模式新增排序行位移动画与落点高亮，拖拽到列表上下边缘时会自动滚动；已运行 `node --check static/js/modules/resource/source-manager.js`、`node --check static/js/modules/app/boot.js`、项目 `compileall`、`git diff --check`，并用 Codex 内置浏览器在 `http://localhost:18080/#tab=settings` 实测排序模式拖拽后 `scrollTop` 增长且草稿顺序变化，验证自动滚动生效；验证后已取消未保存草稿，未改动用户排序。| 下一步：可继续在真实鼠标操作下观察长列表拖拽手感；若还想更顺，可再评估给拖拽中的目标行加占位间距或拖拽镜像。
