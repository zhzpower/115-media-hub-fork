...
`build_resource_channel_profile`（core.py:2565）分析 TG 频道的资源链接类型分布，生成 `link_type_counts`（如 `{"115share": 60, "quark": 30, "magnet": 10}`）。它调用的 `resolve_resource_link_type` → `detect_resource_link_type` 已通过 `resource_linking.py` 中的正则模式**识别全部 13 种链接类型**（含 tianyi、123pan、aliyun），因此新网盘资源被频道抓取后会自动统计。

### 需扩展的部分

| 位置 | 问题 |
|------|------|
| `normalize_resource_provider_filter`（core.py:2608） | 仅识别 `115`/`magnet`/`quark`/`all` 四种过滤值 |
| 前端 `resourceItemMatchesProviderFilter`（core.js:284） | 硬编码 `115→115share`、`quark→quark` 映射 |
| 前端 provider 过滤按钮 | 硬编码 4 个按钮 |

### 改造方式

- `normalize_resource_provider_filter` 改为从 registry 动态匹配：遍历 `list_all()` 的 `link_type` 字段
- 前端过滤逻辑改为从 `providerMeta` 动态构建映射表
- 已在 Phase 1 / Phase 2 中覆盖
